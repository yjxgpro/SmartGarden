...
#define I2C_slkgpio 3
#define I2C_sdagpio 2
#define LED_control1 3
#define LED_control2 4
#define EnoughLight 2000
#define Timer0 0
#define LightPeriod 220
...

{
	 ...
	 **/
	unsigned drdy_gpio = BH1750_DRDY_GPIO;
...
public:
	...
	void setworkmode(uint8_t userdata); // 用户输入工作模式
...
private:
	...
	//uint8_t BH1750_buf[2] = {0}; // BH1750读取的数据, 之后可能传给QT?,先放这里
	static bool recflag = 0;
	bool LEDTri = false; // 根据计算结果决定是否打开开关
	void BH1750initgpio();
	void BH1750WritePoweron();
	void BH1750WriteWorkMode(uint8_t workmode);
	float lightcal(uint8_t *buf); // 在设定分辨率下计算光照强度
	void BH1750dataready();
	static void gpioTimercallFuc(int gpio, int level, uint32_t tick, BH1750 *userdata);
