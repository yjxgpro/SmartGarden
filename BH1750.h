...
class BH1750
{
	...
	extern int8_t BH1750_buf[2]={0};                   // BH1750读取的数据, 之后可能传给QT?,先放这里
	bool LEDTri = false;                    // 根据计算结果决定是否打开开关
    
	void BH1750WritePoweron(uint8_t address, uint8_t subAddress, uint8_t data);
	void BH1750WriteWorkMode(uint8_t address, uint8_t subAddress, uint8_t data);
	uint8_t BH1750RecData(uint8_t address, uint8_t subAddress, uint8_t * dest, uint8_t count);
	void BH1750initgpio();
